...
public class SparkTestNew {



    final static String url = "jdbc:mysql://192.168.0.191:3306/prism";
    final static String user = "root";
    final static String password = "root";

    public static void main(String[] args) {

        //JSONObject.toJSONString( singleMap )
        //System.out.println(JSONObject.parseObject("{'a':1}",Map.class));
        ...
        //mainRDD和newMortgageRDD连接
        JavaPairRDD<String, Tuple2<Iterable<String>, Optional<Iterable<String>>>> join = mainRDD.leftOuterJoin(newAnnualRDD,50);
        ...
        //
        JavaPairRDD<String, Tuple2<String, Optional<Iterable<String>>>> join2 = mainAndAnnualRDD.leftOuterJoin(newMortgageRDD,50);


        //最终RDD
        JavaRDD<String> finalRDD = convertFinalRDD(join2);
        ...
        //JavaEsSpark.saveJsonToEs(finalRDD, "prism/all", ImmutableMap.of("es.mapping.id", "id"));

        //finalRDD.saveAsTextFile("hdfs://192.168.0.191:8020/data/company/");
    }
...
//        new Function<Tuple2<String,Tuple2<String,Iterable<Map<String,Object>>>>, String>() {
//            @Override
//            public String call(Tuple2<String, Tuple2<String, Iterable<Map<String, Object>>>> v1) throws Exception {
//
//
//            }
//        }
    ...
    private static JavaPairRDD<String,Iterable<String>> convertMortgageRDD(JavaPairRDD<String, Iterable<String>> mortRDD) {
        ...
        }).groupByKey(50).filter(new Function<Tuple2<String, Iterable<String>>, Boolean>() {
            @Override
    ...
    private static  JavaPairRDD<String, String> convertMainAndAnnual(JavaPairRDD<String, Tuple2<Iterable<String>, Optional<Iterable<String>>>> join) {

        JavaPairRDD<String, String> stringStringJavaPairRDD = join.mapToPair(new PairFunction<Tuple2<String, Tuple2<Iterable<String>, Optional<Iterable<String>>>>, String, String>() {
            @Override
            public Tuple2<String, String> call(Tuple2<String, Tuple2<Iterable<String>, Optional<Iterable<String>>>> v1) throws Exception {
                String companyId = v1._1();
                Iterator<String> mainIter = v1._2()._1().iterator();
...
//                List<Map<String,Object>> company_mortgage_info=new ArrayList<Map<String, Object>>();
                ...
                while (mainIter.hasNext()) {
                   ...
                   /* if("1".equalsIgnoreCase(companyId)){
                    ...
                    } else if ("company_change_info".equalsIgnoreCase(next.get("tablename").toString())) {
                        company_change_info.add(next);
    ...
    private static JavaPairRDD<String,Iterable<String>> createMortgageRDD(SparkSession spark) {
        ...
        JavaPairRDD<String, String> company_mortgage_info = load20.javaRDD().mapToPair(new PairFunction<Row, String, String>() {
            ...
            public Tuple2<String, String> call(Row row) throws Exception {
                ...
                return new Tuple2<String, String>(returnMap.get("id").toString(), reJson);
            }
        });
    ...
    private static JavaPairRDD<String,Iterable<String>> createMainRDD(SparkSession spark) {
        ...
        load = load.persist(StorageLevel.MEMORY_AND_DISK());
        ...
        Dataset<Row> load41 =spark.sql("select company_id,category_code from company_category ");
        ...
        Dataset<Row> parent_df = load.select(load.col("parent_id"),load.col("name").as("childname"),load.col("regNumber"),load.col("regStatus"),load.col("legalPersonName")).toDF();
        ...
        JavaPairRDD<String, String> company_change_info = load2.javaRDD().mapToPair(new PairFunction<Row, String, String>() {
            private static final long serialVersionUID = -1964392616303257605L;
